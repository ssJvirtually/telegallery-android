...
class UploadWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        // 1. Verify that we have a configured target chat
        ...
        // 4. Scan all photos on the device
        ...
        // 6. Loop and upload new photos sequentially (one-by-one)
        for (photo in photos) {
            val fileKey = photo.uri
            val existing = dao.find(fileKey)

            if (existing == null) {
                TdlibManager.addLog("Worker: backing up photo '${photo.name}'...")
                
                val uploadResult = UploadManager.uploadPhoto(applicationContext, photo, chatId)
