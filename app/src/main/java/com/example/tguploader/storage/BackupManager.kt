...
object BackupManager {
        ...
        try {
            withContext(Dispatchers.IO) {
                ...
                if (uploadResult is TdApi.Message) {
                    ...
                    if (oldMsgId != 0L) {
                        TdlibManager.getClient().send(TdApi.DeleteMessages(chatId, longArrayOf(oldMsgId), true)) { deleteResult ->
                            if (deleteResult is TdApi.Ok) {
                                TdlibManager.addLog("Old SQLite backup message ($oldMsgId) pruned successfully from Telegram.")
                            }
                        }
                    }
                    TdlibManager.addLog("SQLite backup updated successfully to Message ID $newMsgId (Records: $currentCount).")
    ...
    suspend fun restoreDatabase(context: Context, chatId: Long): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            TdlibManager.addLog("Local database is empty. Scanning for remote SQLite backup...")
