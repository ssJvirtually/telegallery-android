# Graph Report - telegallery-calude  (2026-06-11)

## Corpus Check
- 51 files · ~210,011 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1011 nodes · 1592 edges · 71 communities (54 shown, 17 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 84 edges (avg confidence: 0.86)
...
2. `TdlibManager` - 47 edges
3. `Context` - 46 edges
4. `Context` - 35 edges
5. `TGPix — Implementation Plan` - 28 edges
6. `Long` - 26 edges
7. `PhotosGridScreen()` - 24 edges
8. `BackupManager` - 22 edges
9. `List` - 19 edges
10. `CloudPhotoDao` - 19 edges
...
- `Rolling Backup History` --rationale_for--> `backupDatabase()`  [INFERRED]
  BACKUP_STRATEGY_REVIEW.md → app/src/main/java/dev/ssjvirtually/tgpix/storage/BackupManager.kt
- `Atomic Database Copy Locking` --rationale_for--> `backupDatabase()`  [INFERRED]
  BACKUP_STRATEGY_REVIEW.md → app/src/main/java/dev/ssjvirtually/tgpix/storage/BackupManager.kt
...
Cohesion: 0.05
Nodes (32): String, String, String, LocalPhoto, File, Float, String, Bundle (+24 more)
...
Cohesion: 0.24
Nodes (5): Flow, List, Flow, UploadDao, UploadEntity
...
- **Why does `AutoVaultSetupScreen()` connect `Telegram Client Integration` to `Community 32`, `Application Preferences`?**
